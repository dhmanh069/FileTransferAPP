    ...
    if (clients.ContainsKey(receiver))
    {
        ...
        Dispatcher.UIThread.Post(() =>
        {
            lstHistory.Items.Add(
                ...
                {
                    string[] parts = header.Split('|');
                    string userToLogout = parts[1];

                    lock (lockObj)
                    {
                        if (clients.ContainsKey(userToLogout))
                        {
                            clients[userToLogout].Close();
                            clients.Remove(userToLogout);
                        }
                    }

                    Dispatcher.UIThread.Post(() =>
                    {
                        lstClients.Items.Remove(userToLogout);
                        lstHistory.Items.Add($"{userToLogout} logged out");
                    });

                    BroadcastOnlineUsers();
                    
                    break; // Ngắt vòng lặp Listen cho Client này
                }
            }
        }
        catch
